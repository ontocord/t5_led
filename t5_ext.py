...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False, is_decoder=False, layer_id=0):
        super().__init__()
        self.config = config
        ...
        #self.key_value_proj_dim = int(config.hidden_size / config.num_attention_heads)
        self.embed_dim = config.hidden_size

        if not is_decoder:
            self.q_global = self.q # nn.Linear(self.d_model, self.inner_dim, bias=False)
            self.k_global = self.k # nn.Linear(self.d_model, self.inner_dim, bias=False)
            self.v_global = self.v # nn.Linear(self.d_model, self.inner_dim, bias=False)

        attention_window = (
            self.config.attention_window
            if isinstance(self.config.attention_window, int)
            else self.config.attention_window[layer_id]
        )
        assert (
    ...
    def prune_heads(self, heads):
        ...
        self.q_global = prune_linear_layer(self.q_global, index)
        self.k_global = prune_linear_layer(self.k_global, index)
        self.v_global = prune_linear_layer(self.v_global, index)
    ...
    def compute_bias(self, query_length, key_length, is_decoder):
    ...
    def _pad_and_diagonalize(chunked_hidden_states):
        ...
        """
        total_n_heads, num_chunks, window_overlap, hidden_dim = chunked_hidden_states.size()
        chunked_hidden_states = F.pad(
            chunked_hidden_states, (0, window_overlap + 1)
        )  # total_n_heads x num_chunks x window_overlap x (hidden_dim+window_overlap+1). Padding value is not important because it'll be overwritten
        chunked_hidden_states = chunked_hidden_states.view(
            total_n_heads, num_chunks, -1
        )  # total_n_heads x num_chunks x window_overlap*window_overlap+window_overlap
        chunked_hidden_states = chunked_hidden_states[
            :, :, :-window_overlap
        ]  # total_n_heads x num_chunks x window_overlap*window_overlap
        chunked_hidden_states = chunked_hidden_states.view(
            total_n_heads, num_chunks, window_overlap, window_overlap + hidden_dim
        )
    ...
    def _sliding_chunks_query_key_matmul(self, query: torch.Tensor, key: torch.Tensor, window_overlap: int):
        ...
        query = query.transpose(1, 2).reshape(batch_size * n_heads, seq_len, key_value_proj_dim)
        key = key.transpose(1, 2).reshape(batch_size * n_heads, seq_len, key_value_proj_dim)
        query = self._chunk(query, window_overlap)
        key = self._chunk(key, window_overlap)

        # matrix multiplication
        # bcxd: batch_size * n_heads x chunks x 2window_overlap x key_value_proj_dim
    ...
    def _sliding_chunks_matmul_attn_probs_value(
        ...
        chunked_attn_probs = attn_probs.transpose(1, 2).reshape(
            batch_size * n_heads, seq_len // window_overlap, window_overlap, 2 * window_overlap + 1
        )

        # group batch_size and n_heads dimensions into one
        value = value.transpose(1, 2).reshape(batch_size * n_heads, seq_len, key_value_proj_dim)

        # pad seq_len with w at the beginning of the sequence and another window overlap at the end
        padded_value = F.pad(value, (0, 0, window_overlap, window_overlap), value=-1)

        # chunk padded_value into chunks of size 3 window overlap and an overlap of size window overlap
        chunked_value_size = (batch_size * n_heads, chunks_count + 1, 3 * window_overlap, key_value_proj_dim)
        ...
        return context.view(batch_size, n_heads, seq_len, key_value_proj_dim).transpose(1, 2)
    ...
    def _concat_with_global_key_attn_probs(
        ...
        key_vectors_only_global = key_vectors.new_zeros(
            batch_size, max_num_global_attn_indices, self.n_heads, self.key_value_proj_dim
        )
    ...
    def _compute_global_attn_output_from_hidden(
        ...
        global_query_vectors_only_global = (
            ...
            .view(max_num_global_attn_indices, batch_size * self.n_heads, self.key_value_proj_dim)
            .transpose(0, 1)
        )  # (batch_size * self.n_heads, max_num_global_attn_indices, key_value_proj_dim)
        global_key_vectors = (
            global_key_vectors.contiguous().view(-1, batch_size * self.n_heads, self.key_value_proj_dim).transpose(0, 1)
        ...
        ], f"global_attn_output tensor has the wrong size. Size should be {(batch_size * self.n_heads, max_num_global_attn_indices, self.key_value_proj_dim)}, but is {global_attn_output.size()}."

        global_attn_probs = global_attn_probs.view(batch_size, self.n_heads, max_num_global_attn_indices, seq_len)
        global_attn_output = global_attn_output.view(
            batch_size, self.n_heads, max_num_global_attn_indices, self.key_value_proj_dim
        )
    ...
    def forward(
        ...
        if position_bias is None:
            if not self.has_relative_attention_bias:
                ...
                position_bias = self.compute_bias(real_seq_length, key_length, is_decoder=is_decoder)
        ...
        present_key_value_state = (key_states, value_states) if (is_decoder and use_cache) else None
        outputs = (attn_output,) + (present_key_value_state,) + (position_bias,)
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, is_decoder=False, layer_id=0):
        super().__init__()
        self.SelfAttention = T5Attention(config, has_relative_attention_bias=has_relative_attention_bias, is_decoder=is_decoder, layer_id=layer_id)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.EncDecAttention = T5Attention(config, has_relative_attention_bias=False, is_decoder=True, layer_id=layer_id)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, is_decoder=False, layer_id=0):
        ...
        self.layer.append(T5LayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias, is_decoder=is_decoder, layer_id=layer_id))
        if is_decoder: # add_cross_attention:
            self.layer.append(T5LayerCrossAttention(config, layer_id=layer_id))
    ...
    def forward(
        ...
        if past_key_value is not None:
            assert is_decoder, "Only decoder can use `past_key_values`"
        ...
        do_cross_attention = is_decoder and encoder_hidden_states is not None
...
class T5PreTrainedModel(PreTrainedModel):
        ...
        if not hasattr(config, "attention_window"):
            ...
            if False:
                if config.num_layers > 3 and config.num_layers >= 6:
                    ...
                    config.attention_window[1] = 128
                    config.attention_window[2] = 128
                    config.attention_window[3] = 256
                if config.num_layers > 3 and config.num_layers >= 12:
                    config.attention_window[6] = 64
                    config.attention_window[7] = 128
                    config.attention_window[8] = 128
                    config.attention_window[9] = 256
                    config.attention_window[10] = 256
    @property
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            if hasattr(module, "q_global"):
                module.q_global.weight.data.normal_(mean=0.0, std=factor * ((d_model * key_value_proj_dim) ** -0.5))
                module.k_global.weight.data.normal_(mean=0.0, std=factor * (d_model ** -0.5))
                module.v_global.weight.data.normal_(mean=0.0, std=factor * (d_model ** -0.5))
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, is_decoder=False):
        ...
        if hasattr(config, "layer_group"):
            self.layer_group = config.layer_group
        else:
            self.layer_group = [1]*config.num_layers
        assert sum(self.layer_group) == self.config.num_layers, "the sum of the layer group must equal the number of layers"
        self.block = nn.ModuleList()
        layer_id = 0
        for layer_group in self.layer_group:
            layer_module = T5Block(config, has_relative_attention_bias=bool(layer_id == 0), is_decoder=is_decoder, layer_id=layer_id)
            for _ in range(layer_group):
                self.block.append(layer_module)
                layer_id+=1
    ...
    def forward(
        ...
        if use_cache is True:
            assert is_decoder, ":obj:`use_cache` can only be set to `True` if {} is used as a decoder".format(
                self
            )
        ...
        if not is_decoder and global_attention_mask is not None:
        ...
        if is_decoder and encoder_attention_mask is None and encoder_hidden_states is not None:
        ...
        if past_key_values is None:
            past_key_values = [None] * self.config.num_layers #len(self.block)
            #print ("config past_key_values", past_key_values)

        # ourselves in which case we just need to make it broadcastable to all heads.
        if not do_long_former:
            extended_attention_mask = self.get_extended_attention_mask(attention_mask, input_shape, inputs_embeds.device)
        else:
            extended_attention_mask = attention_mask

        if is_decoder and encoder_attention_mask is not None:
        ...
        all_cross_attentions = () if (output_attentions and is_decoder) else None
        position_bias = None
        encoder_decoder_position_bias = None
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            layer_outputs = layer_module(
                ...
                use_cache=use_cache,
                output_attentions=output_attentions,
                compute_relative_attention_bias=bool(i==0),
                is_decoder=is_decoder,
                layer_id=i,
                is_index_masked=is_index_masked,
                is_index_global_attn=is_index_global_attn,
            ...
            if True: # position_bias2 is not layer_outputs[2]:
...
class T5Model(T5PreTrainedModel):
    ...
    def _prune_heads_2(self, heads_to_prune):
        """
        Prunes heads of the model. heads_to_prune: dict of {layer_num: list of heads to prune in this layer} See base
        class PreTrainedModel
        """
        groups = []
        j = 0
        for z, i in enumerate(self.encoder.layer_group):
            groups.append((j, j+i, z))
            j = j+i
        ...
        for layer, heads in heads_to_prune.items():
            for i, group in groups:
                if group is None:
                    continue
                if layer >= group[0] and layer < group[1]:
                    self.encoder.layer[group[2]].attention.prune_heads(heads)
...
if __name__ == "__main__":
    if False:
        ...
        for block  in model.encoder.block:
            block.layer[0].SelfAttention.q_global = block.layer[0].SelfAttention.q
            block.layer[0].SelfAttention.k_global = block.layer[0].SelfAttention.k
            block.layer[0].SelfAttention.v_global = block.layer[0].SelfAttention.v
        model.save_pretrained("t5-small-long")
    if True:
